...
public sealed partial class Answer
{
    ...
    private static Regex ReplacementRegex() => new Regex(@"<<([^>>]+)>>", RegexOptions.Multiline | RegexOptions.Compiled);

    //[GeneratedRegex(]
    private static Regex SplitRegex() => new Regex(@"\[([^\]]+)\]", RegexOptions.Multiline | RegexOptions.Compiled);
}

internal readonly record struct HtmlParsedAnswer(
    string AnswerHtml,
    List<ClientApp.Models.CitationDetails> Citations,
