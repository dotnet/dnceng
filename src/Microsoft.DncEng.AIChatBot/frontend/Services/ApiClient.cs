...
public sealed class ApiClient
{
    ...
    public Task<ClientApp.Models.AnswerResult<M.AskRequest>> AskQuestionAsync(M.AskRequest request) =>
        PostRequestAsync(request, "api/ask");

    public Task<ClientApp.Models.AnswerResult<M.ChatRequest>> ChatConversationAsync(M.ChatRequest request) =>
        PostRequestAsync(request, "api/chat");

    private async Task<AnswerResult<TRequest>> PostRequestAsync<TRequest>(
        TRequest request, string apiRoute) where TRequest : M.ApproachRequest
    {
        ...
        {
            var answer = await response.Content.ReadFromJsonAsync<M.ApproachResponse>();
            return result with
            {
        ...
        {
            var answer = new M.ApproachResponse(
